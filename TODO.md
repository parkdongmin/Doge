...
### 다음에 이어서 할 것
- [ ] 실기기에서 강화 연출 전체 흐름 직접 테스트 (아직 코드만 짜고 실기기 확인 전)
- [ ] 아이디어로만 남겨둔 것: 홀드-투-차지(버튼 길게 눌러서 충전) — 확률엔 영향 없이
      손맛만 추가하는 절충안, 순수 애니메이션보다 구현량 더 필요해서 보류함
- [ ] 강화 말고 다른 화면(탐사/영입/연구소 등)에도 비슷하게 "허전한" 부분이 있는지
      훑어보고 우선순위 정하기 — 아직 강화 화면만 손댐
- [ ] 사운드 에셋 확보되면 진동 대신/같이 효과음 추가 검토
- [ ] 매도를 디테일 화면에서 뺄지 고민했었음 — 목록 화면에도 이미 매도가 있어서
